...
// Load ConsoleDB.txt mappings
   if (SDL_GameControllerAddMappingsFromFile((config.data.res_path + "ConsoleDB.txt").c_str()) == -1)
...
// Initalize SDL Controls
   input.init(config.controls.pad_id, config.controls.keyconfig, config.controls.padconfig, config.controls.analog, config.controls.axis, config.controls.invert, config.controls.asettings);
