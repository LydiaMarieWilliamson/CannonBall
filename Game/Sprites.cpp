...
#include "Game/Data/ZoomTable.hpp"
...
// Get next relevant entry (number of bytes to copy, followed by indexes of sprites)
   ...
   // Copy the required number of bytes
      if (bytes_to_copy != 0) {
         ...
         } while (--bytes_to_copy != 0); // Decrement number of bytes to copy
      }
...
// Unsetting fix_bugs may no longer revert to the original behaviour.
   if (sprite_y2 < 256 || sprite_y1 > 479 || sprite_x2 < x2_bounds || (config.engine.fix_bugs? sprite_x1 >= x1_bounds: sprite_x1 > x1_bounds)) {
      hide_hwsprite(input, output);
      return;
   }
