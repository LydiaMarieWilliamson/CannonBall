...
//
// In the original codebase, each object consists of a 64 byte block of memory.
// Each of these blocks forms part of a jump table that is iterated each tick.
// The sprite referenced the address of the routine it used.
//
// Unfortunately, the system is messy and the usage of each 64 byte entry differs dependent on the game object.
//
// However, the majority are similar.
// And as such, I've tried to convert this structure to a more manageable class.
// For the conversion, rather than dynamically editing a jump table, I've called the sprite routines from the main code.
//
// It's not perfect, but struck a reasonable balance between clarity and being able to debug the conversion.
//
