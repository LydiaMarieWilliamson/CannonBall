...
// Helper function
Int2 ORoad::get_road_y(Num2 index) {
...
Num1 ORoad::get_view_mode() {
   return view_mode;
}

void ORoad::set_view_mode(Num1 mode, bool snap) {
   view_mode = mode;
...
void ORoad::set_default_hscroll() {
   Num4 adr = HW_HSCROLL_TABLE0;
   for (Num2 i = 0; i <= 0x1FF; i++)
...
void ORoad::clear_road_ram() {
   Num4 adr = 0x80000; // start of road ram
   for (Num1 scanline = 0; scanline < S16_HEIGHT; scanline++)
...
#if 0
      Num4 addr = stage_addr + road_data_offset;
#else
      Num4 addr = road_data_offset; // temporary hack for custom data
#endif
      set_tilemap_x(addr);
      setup_x_data(addr);
   }
...
//
// At: 15b0
void ORoad::setup_x_data(Num4 addr) {
   const Int2 x = trackloader.readPath(addr) + trackloader.readPath(addr + 4); // Length 1
   const Int2 y = trackloader.readPath(addr + 2) + trackloader.readPath(addr + 6); // Length 2
// Use Pythagorus' theorem to find the distance/length between x & y
   const Num2 distance = outils::isqrt((x*x) + (y*y));
   const Int2 curve_x_dist = (x << 14)/distance; // Scale up is for fixed point division in create_curve
   const Int2 curve_y_dist = (y << 14)/distance;
...
// Now We Setup The Real Road Data at 60800
   Int4 curve_x_total = 0;
   Int4 curve_y_total = 0;
   Int2 curve_start = 0;
   Int2 curve_end = 0;
   Int2 curve_inc = 0;
   Int2 curve_inc_old = 0;
   Int2 scanline = 0x37E/2; // Index into road_x
...
// We sample 20 Road Positions to generate the road.
   for (Num1 i = 0; i <= 0x20; i++) {
      const Int4 x_next = trackloader.readPath(addr) + trackloader.readPath(addr + 4); // Length 1
      const Int4 y_next = trackloader.readPath(addr + 2) + trackloader.readPath(addr + 6); // Length 2
      addr += 8;
   ...
   // Calculate curve increment and end position
      ...
      Int2 curve_steps = curve_end - curve_start;
      if (curve_steps < 0) return;
      if (curve_steps == 0) continue; // skip_pos
   // X Amount to increment curve by each iteration
      Int4 xinc = (curve_inc - curve_inc_old)/curve_steps;
      Int2 x = curve_inc_old;
      for (Int2 pos = curve_start; pos <= curve_end; pos++) {
         x += xinc;
...
//
// At: 16b6
void ORoad::create_curve(Int2 &curve_inc, Int2 &curve_end, Int4 curve_x_total, Int4 curve_y_total, Int2 curve_x_dist, Int2 curve_y_dist) {
// Note multiplication result should be 32bit, inputs 16 bit
   Int4 d0 = ((curve_x_total >> 5)*curve_y_dist) - ((curve_y_total >> 5)*curve_x_dist);
   Int4 d2 = ((curve_x_total >> 5)*curve_x_dist) + ((curve_y_total >> 5)*curve_y_dist);
   d2 >>= 7;
   Int4 d1 = (d2 >> 7) + 0x410;
...
void ORoad::set_tilemap_x(Num4 addr) {
// d0 = Word 0 + Word 2 + Word 4 + Word 6 [Next 4 x positions]
// d1 = Word 1 + Word 3 + Word 5 + Word 7 [Next 4 y positions]
   Int2 x = trackloader.readPath(&addr);
   Int2 y = trackloader.readPath(&addr);
...
void ORoad::do_road_offset(Int2 *dst_x, Int2 width, bool invert) {
   Int4 car_offset = car_x_bak + width + oinitengine.camera_x_off; // note extra debug of camera x
   Int4 scanline_inc = 0; // Total amount to increment scanline (increased every line)
   Int2 *src_x = road_x; // a0
// Process H-Scroll: Car not central on road 0
// ───────────────────────────────────────────
// The following loop ensures that the road offset becomes before significant the closer it is to the player's car
   if (car_offset != 0) {
      car_offset <<= 7;
      for (Num2 i = 0; i <= 0x1FF; i++) {
         Int2 h_scroll = scanline_inc >> 16;
...
// Get Address of actual road height data
   ...
   Num4 h_addr = trackloader.read_heightmap_table(height_lookup_wrk);
...
//
// At: 1c2c
void ORoad::init_elevation(Num4 &addr) {
...
//
// Note:
// ▪	There is an intriguing bug in this routine, where the actual length of part 2 differs dependent on the speed you are driving at.
//	It's actually possible to elongate the length of hills by driving slower!
//
// Example Data: 0x29AA
// At: 1ce4 (first used at the chicane at stage 1)
void ORoad::init_elevation_delay(Num4 &addr) {
...
// At: 1dac
// Example Data: 0x2A8E
void ORoad::init_elevation_mixed(Num4 &addr) {
   height_delay = trackloader.read16(trackloader.heightmap_data, &addr);
   height_addr = addr;
   do_height_inc = 1;
   height_inc = 0;
...
void ORoad::do_elevation_mixed() {
   Num2 d1 = pos_fine_diff*12;
   height_index += height_inc;
   height_inc = 0;
// Parts 2 & 3 - Delayed Section.
// At: 1e1c
   if (height_index >= 6) {
      Num2 d3 = step_adjust;
...
//
// Part 1:	Height Start Increments from 256 to 512.
//		Change horizon to relevant height over this period.
...
// Example Data: 0x3672
// At: 1eb6
void ORoad::init_horizon_adjust(Num4 &addr) {
   height_addr = addr;
...
void ORoad::do_horizon_adjust() {
   height_step += pos_fine_diff*12;
   Num2 d1 = (height_step/step_adjust);
...
// Road Y Positions (references to this decrement) [Destination]
   ...
   const Int2 next_height_value = trackloader.read16(trackloader.heightmap_data, a1_lookup);
   height_final = (next_height_value*(height_start - 0x100)) >> 4;
// At 1faa
   Int4 horizon_copy = (horizon_base + horizon_offset) << 4;
...
// ────────────────────────────────────────────
   Int2 section_length = section_lengths[length_offset++] - 1;
...
// ────────────────────────────────────────────────────────────────────────
   if (section_length >= 0) {
      for (Num2 i = 0; i <= section_length; i++) {
         total_height += change_per_entry;
         road_y[--y_addr] = (total_height << 4) >> 16;
      }
   }
// ────────────────────────────────────────────────────────────────────────
// Whilst there are still sections of track to write, read the height for the upcoming section
// At: 216c
// ────────────────────────────────────────────────────────────────────────
   if (++counter != 7) {
      read_next_height();
...
// Writing last part of interpolated data
   road_unk[a3_o] = 0;
   Int2 y = trackloader.read16(trackloader.heightmap_data, a1_lookup);
...
// At 1ff2: set_elevation_flag
// Note the way this bug was fixed
// Needed to read the signed value into an Int2 before assigning to a 32 bit value
...
// Writing first part of interpolated data
   change_per_entry -= height_final;
   Int4 horizon_shift = (horizon_base + horizon_offset) << 4;
...
// should be 2400, 2400, 2400, ffffe764, ffffdc00
   ...
   Int4 total_height_wrk = total_height;
...
void ORoad::set_y_horizon() {
   Num4 a0 = 0x200 + road_p1; // a0 = Road Height Positions + Relevant Offset
   Int4 d1 = (horizon_mod*(height_start - 0x100)) >> 4;
   Int4 d2 = ((horizon_base + horizon_offset) << 4) + d1;
   Num4 total_height = 0;
// write_next_y:
   for (Num2 i = 0; i <= 0x1FF; i++) { // (1FF height positions)
      total_height += d2;
      Num4 d0 = (total_height << 4) >> 16;
      road_y[--a0] = d0;
   }
   road_unk[0] = 0;
...
// Read Up/Down Multiplier Word From Lookup Table and set new horizon base value
   horizon_base = (Int4)(trackloader.read16(trackloader.heightmap_data, height_addr));
   if (height_lookup == height_lookup_wrk)
      height_lookup = 0;
...
// ────────────────────────────
   Num4 road_int = 0; // a0
   Num4 road_y_addr = (0 + road_p1); // a1
// read_next
   while (true) {
      Int2 d0 = road_unk[road_int];
      if (d0 == 0) break;
      Int2 d7 = d0 >> 3;
      Int2 d6 = (d7 + d0) - 0x1FF;
      if (d6 > 0) {
   ...
   // At 13e6
      ...
      Int2 d3 = d0 + d6;
      Int2 d4 = d0 + d7;
   // At 1402: Chris - made some edits below here, seems to go rather wrong from here onwards
      d0 -= d7;
      Int2 d5 = d0;
      Num2 a2 = road_y_addr + d5;
   ...
   // At 1426:
      d5 = d0;
      Int4 d2l = (d2 + d0 + d4)*0x5555; // turn d2 into a long
      d2l >>= 16;
      Int4 d1l = (d1 + d0 + (Int2)d2l)*0x5555; // turn d2 into a long
      d1l >>= 16;
      Int4 d3l = (d3 + d4 + (Int2)d2l)*0x5555;
      d3l >>= 16;
      d0 -= (Int2)d1l;
      d1l -= (Int2)d2l;
      d2l -= (Int2)d3l;
...
// This is the final block of data to be output to road hardware
   Num4 addr_dst = 0x400 + road_p1; // [a0]
// Road data in RAM [Source]
   Num4 addr_src = 0x200 + road_p1; // [a1]
// Road Priority Elevation Data [Destination]
   Num4 addr_priority = 0x280 + road_p1;
   Int2 rom_line_select = 0x1FF; // This is the triangular road shape from ROM. Lines 0 - 512.
   Int2 src_this = road_y[--addr_src] >> 4; // source entry #1 / 16 [d0]
   Int2 src_next = 0; // [d4]
   Int2 write_priority = 0; // [d5]
   road_y[--addr_dst] = rom_line_select; // Write source entry counter to a0 / destination
   Int2 scanline = 254; // Loop counter #2 (Scanlines) - Start at bottom of screen
// Draw normal road
// ────────────────
   ...
   // which are a sequential list of numbers when the road is flat
      ...
      // Only draw if within screen area
         if (--scanline <= 0) {
            road_y[addr_priority] = 0;
            road_y[addr_priority + 1] = 0; // Denote end of priority list
            return;
         }
...
// ────────────────────────────────────────
// Note that this is the area above the road, and is detected because the rom line select is negative, and therefore there is no more to read.
   if (rom_line_select <= 0) {
      const Num2 SOLID_FILL = 0x800;
   // d3 = (0x3F | 0x800) = 0x83F [1000 00111111] Sets Solid Fill, Transparent Colour
      const Num2 TRANSPARENT = 0x3F | SOLID_FILL;
      Int2 d7 = 255 - src_next - scanline;
      if (d7 < 0)
...
void ORoad::blit_roads() {
   const Num4 road0_adr = 0x801C0;
   const Num4 road1_adr = 0x803C0;
   switch (road_ctrl) {
   // Both Roads Off
      case ROAD_OFF:
      return;
   ...
   // Road 1
      case ROAD_R1:
   // Road 1 (Road Split. Invert Road 1)
      case ROAD_R1_SPLIT:
         blit_road(road1_adr);
         hwroad.write_road_control(3);
      break;
   // Both Roads (Road 0 Priority) [DEFAULT]
      case ROAD_BOTH_P0:
   // Both Roads (Road 0 Priority) (Road Split. Invert Road 1)
      case ROAD_BOTH_P0_INV:
         blit_road(road0_adr);
         blit_road(road1_adr);
         hwroad.write_road_control(1);
      break;
   // Both Roads (Road 1 Priority)
      case ROAD_BOTH_P1:
   // Both Roads (Road 1 Priority) (Road Split. Invert Road 1)
      case ROAD_BOTH_P1_INV:
         blit_road(road0_adr);
         blit_road(road1_adr);
...
void ORoad::blit_road(Num4 a0) {
   Num4 a2 = 0x400 + road_p2;
// Write 0x1A0 bytes total Src: (0x260 - 0x400) Dst: 0x1C0
   for (Int2 i = 0; i <= 13; i++) {
      a0 -= 2;
      hwroad.write16(a0, road_y[--a2]);
      a0 -= 2;
      hwroad.write16(a0, road_y[--a2]);
      a0 -= 2;
      hwroad.write16(a0, road_y[--a2]);
...
void ORoad::output_hscroll(Int2 *src, Num4 dst) {
   const Int4 d6 = 0x654;
   Num4 src_addr = 0;
// Copy 16 bytes
   for (Int4 i = 0; i <= 0x3F; i++) {
      Int2 d0h = -src[src_addr++] + d6;
      Int2 d0l = -src[src_addr++] + d6;
      Int2 d1h = -src[src_addr++] + d6;
      Int2 d1l = -src[src_addr++] + d6;
      Int2 d2h = -src[src_addr++] + d6;
      Int2 d2l = -src[src_addr++] + d6;
      Int2 d3h = -src[src_addr++] + d6;
      Int2 d3l = -src[src_addr++] + d6;
...
// Scroll stripe data over road based on fine position
   Num4 pos_fine_copy = (pos_fine&0x1F) << 10;
   Num4 src = ROAD_BGCOLOR + pos_fine_copy;
   Num4 dst = HW_BGCOLOR;
// Copy 1K
   for (int i = 0; i < 32; i++) {
