...
// ------------------------------------------------------------------------
   ...
   data.file_scores_jap = data.save_path + "TopPlayJap.xml";
   data.file_ttrial = data.save_path + "TopLaps.xml";
...
// Additional Level Objects
   ...
   engine.fix_bugs_backup = engine.fix_bugs = pt_config.get("engine.fix_bugs", 1) != 0;
...
// Save stuff
   ...
   pt_config.put("continuous.traffic", cont_traffic), ttrial.laps = pt_config.get("time_trial.laps", 5);
   ttrial.traffic = pt_config.get("time_trial.traffic", 3);
   cont_traffic = pt_config.get("continuous.traffic", 3);
   try {
...
// Create empty property tree object
   ...
   try {
      ...
      std::cout << "Error saving Tops: " << e.what() << "\n";
   }
}
