...
//
// This tells DirectInput that we will be passing a DIJOYSTATE structure to IDirectInputDevice8::GetDeviceState().
// Even though we won't actually do it in this sample.
// But setting the data format is important so that the DIJOFS_* values work properly.
...
// Exclusive access is required in order to perform force feedback.
   if (FAILED(hr = g_pDevice->SetCooperativeLevel(hDlg, DISCL_EXCLUSIVE | DISCL_BACKGROUND))) {
      return hr;
   }
...
// Desc:	Callback function for enumerating the axes on a joystick and counting each force feedback enabled axis
BOOL CALLBACK EnumAxesCallback(const DIDEVICEOBJECTINSTANCE *pdidoi, VOID *pContext) {
...
// Desc:	Called once for each enumerated force feedback device.
...
// So continue enumerating
   if (FAILED(hr))
      return DIENUM_CONTINUE;
// We successfully created an IDirectInputDevice8.
// So stop looking for another one.
   g_pDevice = pDevice;
   return DIENUM_STOP;
}
